...
5. **If the user says yes:** overwrite only *tracked* local copies that are blocking or out of sync with the upstream version (e.g. `git checkout -- <file>` for a conflicting tracked file, or remove/replace an untracked file colliding with a new upstream one), run `git pull --ff-only`, then run `bash setup.sh` to create any reference files that are still missing. **Never touch `reference/*.md` or `custom-instructions.md` as part of this** — they are gitignored, `git` cannot see or overwrite them, and `setup.sh` only creates them when absent; it never overwrites an existing one. Your cached tenant data (entity IDs, cost baselines, etc.) is always safe through this process. Then proceed to re-read `CLAUDE.md` and `.github/copilot-instructions.md` from disk, since they likely just changed.
...
|-----------|--------|-------------|
| `skills/dt-dql-essentials/SKILL.md` | **REQUIRED** — DQL syntax, pitfalls, data objects, query cost tuning | Before writing ANY DQL query |
| `skills/dt-migration/SKILL.md` | Classic/Gen2 entity → Smartscape migration | Migrating old DQL queries |
...
| `skills/dt-obs-analytics/SKILL.md` | Davis analyzers on dashboards/notebooks | User references a specific dashboard/notebook by URL, UUID, or name |
| `skills/dt-obs-genai/SKILL.md` | GenAI/LLM apps — token cost, agent loops, guardrails, evals | LLM app debugging, AI cost analysis |
| `skills/dt-obs-ext-monitors/SKILL.md` | 3rd-party test/monitor result ingestion | Sending external synthetic/CI check results to Grail |
...
| `skills/dt-obs-compliance-assistant/SKILL.md` | EU DORA compliance posture, CIF health, ICT risk | Compliance score/incident lifecycle questions |
...
| Skill | Domain | When to Load |
|-----------|--------|-------------|
| `skills/dt-platform-costs/SKILL.md` | Tenant's ACTUAL DPS billing/consumption data | "how much did queries cost", chargeback/showback of Dynatrace spend |
